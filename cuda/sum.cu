...
__global__ void sum_v1(int *output, int *input, const int count) {
...
__global__ void sum_v2(int *output, int *input, const int count) {
    // Every thread block is responsible for twice as many elements as it has threads.
    unsigned int segment = blockIdx.x * blockDim.x * 2;
...
__global__ void sum_v3(int *output, const int *input, const int count) {
    // Every thread block is responsible for twice as many elements as it has threads.
    unsigned int segment = blockIdx.x * blockDim.x * 2;
...
__global__ void sum_v4(int *output, const int *input, const int count) {
...
__global__ void sum_v5(int *output, const int *input, const int count) {
...
__global__ void sum_v6(int *output, const int *input, const int count) {
    __shared__ int s_mem[BLOCK_SIZE];
...
__global__ void sum_warp(int *output, const int *input, const int count) {
    // Grid stride loop to load data
    int val = 0;
    for (int i = blockDim.x * blockIdx.x + threadIdx.x; i < count; i += blockDim.x * gridDim.x) {
        val += input[i];
...
int main() {
    ...
    for (int i = 0; i < NUM_REPS; i++) {
        ...
        d_output.fill_zero();
        sum_v1<<<numBlocks, numThreadsPerBlock>>>(d_output.data(), d_input.data(), n);
    }
    ...
    for (int i = 0; i < NUM_REPS; i++) {
        d_input.copy_from_host(h_input.data(), n);
        d_output.fill_zero();
        sum_v2<<<numBlocks, numThreadsPerBlock>>>(d_output.data(), d_input.data(), n);
    }
    CUDA_OK(cudaGetLastError());
    ...
    for (int i = 0; i < NUM_REPS; i++) {
        d_output.fill_zero();
        sum_v3<<<numBlocks, numThreadsPerBlock>>>(d_output.data(), d_input.data(), n);
    }
    CUDA_OK(cudaGetLastError());
    ...
    for (int i = 0; i < NUM_REPS; i++) {
        d_output.fill_zero();
        sum_v4<<<numBlocks1, numThreadsPerBlock1>>>(d_output.data(), d_input.data(), n);
    }
    CUDA_OK(cudaGetLastError());
    ...
    for (int i = 0; i < NUM_REPS; i++) {
        d_output.fill_zero();
        sum_v5<<<GRID_SIZE, BLOCK_SIZE>>>(d_output.data(), d_input.data(), n);
    }
    CUDA_OK(cudaGetLastError());
    ...
    for (int i = 0; i < NUM_REPS; i++) {
        d_output.fill_zero();
        sum_v6<<<GRID_SIZE, BLOCK_SIZE>>>(d_output.data(), d_input.data(), n);
    }
    CUDA_OK(cudaGetLastError());
    ...
    for (int i = 0; i < NUM_REPS; i++) {
        d_output.fill_zero();
        sum_warp<<<GRID_SIZE, BLOCK_SIZE>>>(d_output.data(), d_input.data(), n);
    }
    CUDA_OK(cudaGetLastError());
